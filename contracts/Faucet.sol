...
contract Faucet is EventStore  {
  ...
  function Faucet(string _name, address _creator) {
    ...
    blockLimit = 0;
    emitEvent('FAUCET_CREATED', _creator, 0, _name);
  }
  ...
  // Getters and Setters
	function setBlockLimit(uint limit) onlyCreator
    returns (bool)
  {
    blockLimit = limit;
    return true;
    ...
    returns (uint)
  {
	...
	function setSendAmount(uint256 val)
    onlyCreator
    returns (bool)
  {
    sendAmount = val;
  ...
  function addRequestorAddress(address _requestor) public {
    ...
    emitEvent('FAUCET_ACCESS_REQUESTED', _requestor, 0, '');
  }
    ...
    onlyCreator
  {
    ...
    emitEvent('FAUCET_ACCESS_GRANTED', _requestor, 0, '');
  }
    ...
    onlyCreator
  {
    ...
    if (!authorizedAddressesMapping[_requestor])
      throw;
    authorizedAddressesMapping[_requestor] = false;
    emitEvent('FAUCET_ACCESS_REVOKED', _requestor, 0, '');
  }
